          ...
          <Splide options={splideOptions}>
            {news?.map((val, i) => (
              <SplideSlide key={i} className="mb-0.5">
                <div
                  className="relative grid items-center gap-4 pb-2 rounded-lg 
                ...
                >
                  ...
                  <div className="flex items-center justify-center px-4 w-full">
                    <a target="_blank" role={"button"} className="w-full bg-gradient-to-b from-slate-900 to-black shadow-md shadow-black
                    text-center text-slate-100 py-1.5 button-theme" href={val.url}>{val.btn}</a>
                  </div>
                </div>
              </SplideSlide>
            ))}
