...
const Items = ({ ifExists, id, color, shadow, title, text, img, btn, rating, price }) => {
  ...
  return (
    <div className={`relative bg-gradient-to-b ${color} ${shadow} ${ifExists ? "justify-items-start" : "justify-items-center"} grid items-center 
    rounded-xl py-4 px-5 transition-all duration-700 ease-in-out w-full hover:scale-105`}>
      <div className={`grid items-center ${ifExists ? "justify-items-start" : "justify-items-center"}`}>
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => onAddToCart()} type="button" className="bg-white/90 blur-effect-theme button-theme p-0.5
          shadow shadow-sky-200"><ShoppingBagIcon className="icon-style text-slate-900"/></button>
          <button onClick={() => {onAddToCart(); onCartToggle();}} type="button" className="bg-white/90 blur-effect-theme button-theme px-2 py-1
          shadow shadow-sky-200 text-sm text-black">{btn}</button>
        </div>
      </div>
